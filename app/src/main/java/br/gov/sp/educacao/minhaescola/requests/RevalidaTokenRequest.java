...
public class RevalidaTokenRequest {
    ...
    //TODO salvar token no banco e selecionar perfil quando for responsável

    public String executeRequest(Context ctx) {

        String username, password;

        MyPreferences mPref = new MyPreferences(ctx);
        UsuarioQueries usuarioQueries = new UsuarioQueries(ctx);
        ...
        if(mPref.getPerfilSelecionado() == MyPreferences.PERFIL_RESPONSAVEL) {
            ...
            resp = usuarioQueries.getResponsavel();

            username = resp.getLogin();
            password = resp.getSenha();
        }
        else {
        ...
        try {
            ...
            if (statusCode == 201
                ...
                String loginResponse = ConnectionReader
                        .readStringFromHttpsURLConnection(true, httpsURLConnection);
                ...
                if (loginJsonResponse.has("Erro")) {
                    ...
                    //Se não houver mensagem de erro retorna o token
                    String token = loginJsonResponse.getString("Token");

                    usuarioQueries.atualizarToken(token);

                    if(mPref.getPerfilSelecionado() == MyPreferences.PERFIL_RESPONSAVEL) {

                        executeSelecionarPerfilRequest(token, ctx);
                    }
                ...
                String loginResponse = ConnectionReader
                        .readStringFromHttpsURLConnection(false, httpsURLConnection);
